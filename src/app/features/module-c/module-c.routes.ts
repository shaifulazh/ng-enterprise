...
import { Injectable, inject, Component, OnInit, signal } from '@angular/core';
...
@Component({
  ...
  template: `
    <div class="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Module C — Configuration</h1>
      ...
      @if (saved()) {
        <div class="p-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg text-sm text-emerald-700 dark:text-emerald-400 flex items-center gap-2">
      ...
      <div class="card divide-y divide-slate-100 dark:divide-slate-700/60">
        @if (loading()) {
          @for (_ of [1,2,3,4]; track $index) {
            <div class="p-5 flex justify-between items-center animate-pulse">
              <div class="space-y-2">
          ...
          @for (entry of localConfig; track entry.key; let i = $index) {
            <div class="p-5 flex items-center justify-between gap-6">
              <div class="min-w-0 flex-1">
                <p class="text-sm font-medium text-slate-900 dark:text-white">{{ entry.label }}</p>
              ...
              @if (entry.type === 'toggle') {
                <button
                  ...
                  [class.bg-slate-300]="entry.value !== 'true'"
                ...
                <input
                  type="text"
                  class="input-field w-40 shrink-0"
                  [(ngModel)]="localConfig[i].value"
                />
              }
            </div>
