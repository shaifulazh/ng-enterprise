...
@Component({
  ...
  template: `
    <header class="h-16 bg-white dark:bg-surface-dark-secondary border-b border-slate-200 dark:border-slate-700/60 flex items-center px-4 gap-3 shrink-0 z-10">
      ...
      <!-- Page title (populated by router) -->
      ...
      <div class="flex items-center gap-1">
        ...
        <button
          ...
          [attr.aria-label]="isDark() ? t().settings.lightMode : t().settings.darkMode"
        >
          @if (isDark()) {
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707
                       m12.728 0l-.707-.707M6.343 6.343l-.707-.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/>
            </svg>
          } @else {
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Notifications -->
        <button class="btn-ghost p-2 relative" [attr.aria-label]="t().nav.notifications">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="relative ml-1">
          ...
          @if (profileOpen()) {
            <div
              class="absolute right-0 top-full mt-1.5 w-56 card py-1 z-50 animate-fade-in"
              role="menu"
            >
              <!-- User info header -->
              <div class="px-3 py-2.5 border-b border-slate-100 dark:border-slate-700">
                <p class="text-xs text-slate-400">{{ t().common.signedInAs }}</p>
                <p class="text-sm font-medium text-slate-900 dark:text-white truncate mt-0.5">{{ displayName() }}</p>
              </div>

              <!-- Menu items -->
              @for (item of getProfileMenuItems(); track item.label) {
                <button
                  ...
                  [class.text-red-600]="item.action === 'logout'"
                  [class.dark:text-red-400]="item.action === 'logout'"
                  role="menuitem"
                  (click)="handleProfileAction(item.action)"
                >
                  <span [innerHTML]="item.icon" class="text-slate-400 shrink-0"></span>
                  {{ item.label }}
                </button>
              }
            </div>
...
export class TopNavbarComponent {
  ...
  toggleMobile(): void { this.store.dispatch(LayoutActions.toggleMobileMenu()); }

  toggleDarkMode(): void {
    this.store.dispatch(SettingsActions.setTheme({ theme: this.isDark() ? 'light' : 'dark' }));
  }

  handleProfileAction(action: string): void {
    this.profileOpen.set(false);
    if (action === 'logout') this.store.dispatch(AuthActions.logout());
  }
